  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ error: 'Utilisateur non trouvé' });
	    ...
	    // Enregistrer la visite (optionnel, on verra plus tard)
...
const updateProfile = async (req, res) => {
  try {
    const allowedUpdates = ['firstName', 'lastName', 'email', 'bio', 'gender', 'sexualPreferences', 'locationCity'];
    const updates = {};
...
module.exports = { getMyProfile, getUserProfile, updateProfile };
